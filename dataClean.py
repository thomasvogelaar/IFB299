...
#open the csv file
with open('Data/dataCleaning/dataInStore.csv', 'r') as store_csv_file:
    store_csv_reader = csv.DictReader(store_csv_file)
    with open('Data/dataCleaning/dataInStore.csv', 'r') as car_csv_file:
        car_csv_reader = csv.DictReader(car_csv_file)
        with open('Data/dataCleaning/dataInStore.csv', 'r') as customer_csv_file:
            customer_csv_reader = csv.DictReader(customer_csv_file)
            with open('Data/dataCleaning/dataInStore.csv', 'r') as transaction_csv_file:
                transaction_csv_reader = csv.DictReader(transaction_csv_file)
           
                #open the empty csv file 
                with open('Data/dataCleaning/Store.csv', 'w') as store_file:
                    store_writer = csv.DictWriter(store_file, fieldnames=store, lineterminator='\n')
                    with open('Data/dataCleaning/Car.csv', 'w') as car_file:
                        car_writer = csv.DictWriter(car_file, fieldnames=car, lineterminator='\n')
                        with open('Data/dataCleaning/Customer.csv', 'w') as customer_file:
                            customer_writer = csv.DictWriter(customer_file, fieldnames=customer, lineterminator='\n')    
                            with open('Data/dataCleaning/Transaction.csv', 'w') as transaction_file:
...
# import store data
storeData = csv.reader(open("Data/dataCleaning/Store.csv"), delimiter = ",")
...
#import car data
carData = csv.reader(open("Data/dataCleaning/Car.csv"), delimiter = ",")
for row in carData:
    if row[0] != 'create_data' and row[0] is not None and row[0] != 'NULL':
        newcar = Car()
        newcar.make = row[0]
...
customerData = csv.reader(open("Data/dataCleaning/Customer.csv"), delimiter = ",")
...
transactionData = csv.reader(open("Data/dataCleaning/Transaction.csv"), delimiter = ",")
